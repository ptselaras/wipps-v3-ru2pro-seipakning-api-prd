...
namespace WIPPS_API_3._0.Controllers
{
    ...
    [ApiController]
    public class RequirementsController : ControllerTrait<Requirement>
    {
        ...
        private static string plural = "requirements";
        private static string table = "requirements";
        private static string[] fields = { "id", "created_at", "updated_at" };
        private static string cond = "slug";

        public RequirementsController(SafetymanContext context, IUriService uriService, IWebHostEnvironment hostingEnvironment) : base(context, hostingEnvironment, plural, cond, table, form, fields)
        {
            _context = context;
        ...
        [HttpGet]
        public async Task<ActionResult<object>> GetRequirements(
        ...
        )
        {
            ...
            var query = _context.Requirements.FromSqlRaw(sQuery).ToList();
        ...
        [HttpGet("{slug}")]
        public async Task<ActionResult<Requirement>> GetRequirement(string slug)
        {
        ...
        [HttpPut("{slug}")]
        public async Task<ActionResult<object>> PutRequirement(string slug, RequestItem request)
        {
            ...
            try
            {
                ...
                _context.Requirements.Update(item);
        ...
        // more details, see https://go.microsoft.com/fwlink/?linkid=2123754.
        [HttpPost]
        public async Task<ActionResult<object>> PostRequirement(RequestItem request)
        {
            var validator = new Validator();
            var result = validator.Validate(request);
            ...
            try
            {
                ...
                requirement.Slug = StringExtensions.Slugify(request.name);
                requirement.RefineryId = refinery_id;
            ...
            data.slug = requirement.Slug;
            data.created_at = requirement.CreatedAt;
            data.updated_at = requirement.UpdatedAt;
            data.refinery_id = requirement.RefineryId;
        ...
        public async Task<ActionResult<object>> GetList()
        {
            ...
            var query = await _context.Requirements.Where(x => x.RefineryId == user.RefineryId).ToListAsync();
        ...
        [HttpDelete("{slug}")]
        public async Task<ActionResult<object>> DeleteRequirement(string slug)
        {
            var query = _context.Requirements
                            .Where(x => x.Slug.ToLower() == slug.ToLower())
                            .FirstOrDefault();
            ...
            try
            {
                _context.Requirements.Remove(query);
                await _context.SaveChangesAsync();
